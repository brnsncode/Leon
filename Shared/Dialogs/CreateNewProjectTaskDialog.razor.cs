...
{
    public partial class CreateNewProjectTaskDialog
    {
        /*  
       ...
       */
        ...
        private MudDropContainer<CreateNewProjectTaskDialog> _dropContainer;
        ...
        private bool linkToPlanner { get; set; } = true;
        ...
        public string capacityPercentage { get; set; } = "5";
        ...
        public async void CreateNewProjectTask(MouseEventArgs e)
        {
            taskName = taskName;
            ...
            try
            {
                ...
                if(filteredProjectId == 0)
                {
                    ...
                    _assignedTaskService.CreateNewProjectTask(taskName, filteredProjectId, Int32.Parse(projectId), filteredResourceId, capacityPercentage, passedStatus, isReminder, linkToPlanner);

                }
                else
                {
                    _assignedTaskService.CreateNewProjectTask(taskName, filteredProjectId, filteredOperationId, filteredResourceId, capacityPercentage, passedStatus, isReminder, linkToPlanner);

                    //get newly created project taskid 
                    newTaskId = _projectService.GetTaskIdByTaskName(taskName, filteredResourceId);
                }
                if (newNote != null)
                {
                    var response = _assignedTaskService.CreateNewTaskNote(newTaskId, filteredResourceId, newNote);
                    if (response == "empty")
                    {
                        //Close Dialog
            ...
            try
            {
                ...
                if (createPlannerTask)
                {
                    ...
                    var result = await GraphServiceClient.Planner.Tasks.Request().AddAsync(requestBody);
                }
                else
                {
                    // skip
